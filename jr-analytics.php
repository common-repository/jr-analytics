<?php
/*
Plugin Name: JR Analytics
Plugin URI: http://www.jakeruston.co.uk/2010/04/wordpress-plugin-jr-analytics/
...
add_action('admin_menu', 'jr_Analytics_add_pages');

// action function for above hook
function jr_Analytics_add_pages() {
    add_options_page('JR Analytics', 'JR Analytics', 'administrator', 'jr_Analytics', 'jr_Analytics_options_page');
}
...
register_activation_hook(__FILE__,'analytics_choice');
if (!function_exists("jr_Analytics_refresh")) {
function jr_Analytics_refresh() {
update_option("jr_submitted_Analytics", "0");
}
}

function analytics_choice () {
if (get_option("jr_Analytics_links_choice")=="") {

if (_iscurlinstalled()) {
$pname="jr_Analytics";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_Analytics", "1");
wp_schedule_single_event(time()+172800, 'jr_Analytics_refresh'); 
} else {
...
update_option("jr_Analytics_links_choice", $content);
}
}

if (get_option("jr_analytics_link_personal")=="") {
$content = curl_get_contents("http://www.jakeruston.co.uk/p_pluginslink4.php");

update_option("jr_analytics_link_personal", $content);
}
}

// jr_Analytics_options_page() displays the page content for the Test Options submenu
function jr_Analytics_options_page() {
...
<?php
    ...
    echo "<h2>" . __( 'JR Analytics Plugin Options', 'mt_trans_domain' ) . "</h2>";
$blog_url_feedback=get_bloginfo('url');
...
$linkper=utf8_decode(get_option('jr_analytics_link_personal'));

if (get_option("jr_Analytics_link_newcheck")=="") {
$pieces=explode("</a>", get_option('jr_Analytics_links_choice'));
...
if (get_option("jr_submitted_Analytics")=="0") {
$pname="jr_Analytics";
$url=get_bloginfo('url');
$content = curl_get_contents("http://www.jakeruston.co.uk/plugins/links.php?url=".$url."&pname=".$pname);
update_option("jr_submitted_Analytics", "1");
...
wp_schedule_single_event(time()+172800, 'jr_Analytics_refresh'); 
} else if (get_option("jr_submitted_Analytics")=="") {
$pname="jr_Analytics";
$url=get_bloginfo('url');
$current=get_option("jr_Analytics_links_choice");
...
wp_schedule_single_event(time()+172800, 'jr_Analytics_refresh'); 
  ...
  $pshow = "<p style='font-size:x-small'>Analytics Plugin created by ".$linkper." - ".stripslashes(get_option('jr_Analytics_links_choice'))."</p>";
  echo $pshow;
}
